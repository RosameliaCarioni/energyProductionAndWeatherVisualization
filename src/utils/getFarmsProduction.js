import { endpoint } from '@/utils/endpoint'
  ...
  export async function getProductionInHour(id, Year, Month, Day, Hour) {
    ...
    //console.log("energy production ", energyProductions)
    return energyProductions
  }



  export async function getProductionAfterIceLossInHour(id, Year, Month, Day, Hour) {
    const response = await fetch(`${endpoint}/farmsProduction/${id}/${Year}/${Month}/${Day}`)
    ...
    const formatted_data = await response.json()
    const energyProductions = formatted_data.farms.filter(item => item.Hour === Hour.toString()).map(item => item.EnergyAfterIceLoss);
    return energyProductions
  }
