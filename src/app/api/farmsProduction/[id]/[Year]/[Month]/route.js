/**
 * Retrieves a farm based on the provided ID and YEAR and Month.
 *
 * @param {Object} req - The request object.
 * @param {Object} params - The route parameters.
 * @param {string} params.Month - The Month of production
 *
...
export async function GET(req, { params }) {
  try {
    const farms = energy_production.data.filter(item => (item.id === params.id) && (item.Year === params.Year) && (item.Month === params.Month))
    if (farms.length === 0) {
